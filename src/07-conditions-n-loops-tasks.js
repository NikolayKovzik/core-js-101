...
function isInsideCircle(circle, point) {
  const cathetX = circle.center.x - point.x;
  const cathetY = circle.center.y - point.y;
  return Math.sqrt(cathetX ** 2 + cathetY ** 2) < circle.radius;
}
...
function getDigitalRoot(num) {
  const sum = `${num}`.split('').reduce((res, digit) => res + +digit, 0);
...
function getMatrixProduct(m1, m2) {
  ...
  for (let row = 0; row < m1.length; row += 1) {
    matrix[row] = new Array(m2[0].length);
